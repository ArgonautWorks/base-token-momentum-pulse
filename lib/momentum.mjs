...
async function loadFromRpc(endpoint, input, options) {
  ...
  return {
    ...
    tokens: rankings.slice(0, input.limit),
  };
}
...
export function createMomentumLoader({
  ...
  async function refresh(input) {
    ...
    for (const endpoint of rpcUrls) {
      try {
        const snapshot = await loadFromRpcImpl(endpoint, input, { fetchImpl, now, timeoutMs });
  ...
  return async function loadMomentum(rawInput = {}) {
    const input = parseMomentumInput(rawInput);
    const current = now();
    if (cache && current - Date.parse(cache.observed_at) <= ttlMs) return projectSnapshot(cache, input, current, false);
    if (!inFlight) inFlight = refresh(input).finally(() => { inFlight = null; });
    try {
      return projectSnapshot(await inFlight, input, now(), false);
    } catch (error) {
      if (cache && current - Date.parse(cache.observed_at) <= staleTtlMs) return projectSnapshot(cache, input, current, true);
      throw error;
    }
  };
}

export const loadMomentum = createMomentumLoader();
